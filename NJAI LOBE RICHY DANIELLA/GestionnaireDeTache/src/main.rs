...
fn main() {
    ...
    let tache0 = creer_une_tache(String::from("aller en balade"), 1);
    afficher_la_tache(tache0);

    // Correction 2 : extraire_titre -> extraire_du_titre
    let tache1 = creer_une_tache(String::from("Faire une course"), 2);
    let titre_tache1 = extraire_du_titre(tache1);
    println!("Titre de la tâche 1: {}", titre_tache1);

    let tache2 = creer_une_tache(String::from("regarder un film"), 3);
